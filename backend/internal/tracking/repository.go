...
func (r *Repository) Create(ctx context.Context, event *TrackingEvent) error {
	now := time.Now().UTC()
...
func (r *Repository) GetByID(ctx context.Context, id int64) (*TrackingEvent, error) {
...
func (r *Repository) List(ctx context.Context, orderCode, driverCode string, offset, limit int) ([]TrackingEvent, error) {
...
func (r *Repository) GetByOrder(ctx context.Context, orderCode string) ([]TrackingEvent, error) {
	query := `SELECT id, order_code, driver_code, status_update, lat, lng, note, timestamp
...
func (r *Repository) Update(ctx context.Context, id int64, event *TrackingEvent) error {
	query := `UPDATE tracking_events SET order_code = ?, driver_code = ?, status_update = ?, lat = ?, lng = ?, note = ?, timestamp = ? WHERE id = ?`
	result, err := r.DB.ExecContext(ctx, query,
		event.OrderCode, event.DriverCode, event.StatusUpdate, event.Lat, event.Lng, event.Note,
		event.Timestamp, id)
	if err != nil {
		return err
	}
