...
func (r *Repository) List(ctx context.Context, orderCode, driverCode string, offset, limit int) ([]entity.TrackingEvent, error) {
	...
	if events == nil { events = []entity.TrackingEvent{} }
	return events, nil
}
...
func (r *Repository) Count(ctx context.Context, orderCode, driverCode string) (int, error) {
	var conditions []string
	var args []interface{}
	if orderCode != "" { conditions = append(conditions, "order_code = ?"); args = append(args, orderCode) }
	if driverCode != "" { conditions = append(conditions, "driver_code = ?"); args = append(args, driverCode) }
	whereClause := ""
...
func (r *Repository) Update(ctx context.Context, id int64, event *entity.TrackingEvent) error {
	...
	if rows == 0 { return apierrors.ErrNotFound }
	event.ID = id
	return nil
}
