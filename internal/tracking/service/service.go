...
func (s *Service) Create(ctx context.Context, req *dto.CreateTrackingEventRequest) (*entity.TrackingEvent, error) {
	...
	event := &entity.TrackingEvent{
		OrderCode: req.OrderCode, DriverCode: req.DriverCode, StatusUpdate: req.StatusUpdate,
		Lat: req.Lat, Lng: req.Lng, Note: req.Note, Timestamp: time.Now().UTC(),
	}
...
func (s *Service) Update(ctx context.Context, id int64, req *dto.UpdateTrackingEventRequest) (*entity.TrackingEvent, error) {
	...
	if req.OrderCode != nil {
		if *req.OrderCode == "" { return nil, fmt.Errorf("%w: order_code must not be empty", apierrors.ErrBadRequest) }
		event.OrderCode = *req.OrderCode
	}
	if req.DriverCode != nil {
		if *req.DriverCode == "" { return nil, fmt.Errorf("%w: driver_code must not be empty", apierrors.ErrBadRequest) }
		event.DriverCode = *req.DriverCode
	}
	if req.StatusUpdate != nil {
		if *req.StatusUpdate == "" { return nil, fmt.Errorf("%w: status_update must not be empty", apierrors.ErrBadRequest) }
		event.StatusUpdate = *req.StatusUpdate
	}
	if req.Lat != nil { event.Lat = *req.Lat }
	if req.Lng != nil { event.Lng = *req.Lng }
	if req.Note != nil { event.Note = *req.Note }
	event.Timestamp = time.Now().UTC()
	if err := s.Repo.Update(ctx, id, event); err != nil { return nil, err }
	return event, nil
}
